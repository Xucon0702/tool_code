...
void TransferRadius2FrontAxleAngle()
{
    ...
    for(int i = 170; i <=500; i+=30)
    {
        ...
        qDebug("{%d*PI/180, %f, %f, %f, %f},",i,thetaLF,thetaLR,thetaRF,thetaRR);
    }
...
//    radiusSet.push_back(fR);
    ...
    qDebug("MaxR, MinR is %f, %f,delta is %f", MaxR, MinR, MaxR - MinR);
...
//      m_nRunningDirectionRealTime 1:Forward -1:Back 0:Static
        ...
        {
            qDebug("Vehicle dis calculate error!!!");
        }
        if((nGear==2&&nPulseDirection==1)||(nGear==4&&nPulseDirection==2)&&fDeltaDis>0.001)
        {
            qDebug("车辆溜车！！！%f",fDeltaDis);
        }
//        qDebug("nGear%d,nPulseDirection%d,m_nRunningDirectionRealTime%d",nGear,nPulseDirection,m_nRunningDirectionRealTime);

        if((fTireRLDeltaDis> 5 * 0.0216)||(fTireRRDeltaDis> 5 * 0.0216))
        {
            qDebug("fTireRLDeltaDis %f _____fTireRRDeltaDis %f ",fTireRLDeltaDis ,fTireRRDeltaDis);
        }
...
//    qDebug("nRunningDirectionRealTime!!!%d;fTireFRDeltaDis%f",nRunningDirectionRealTime,fTireFRDeltaDis);
...
//    qDebug("fTotalDisCalByFourWheel%f,fTotalDisCalByRearWheel%f",fTotalDisCalByFourWheel,fTotalDisCalByRearWheel);
    current_angle=mod2pi(current_angle);
    return 0;
}
...
void CoordinateTransformationFromWorldToBodyPos(const LocationPoint &body, LocationPoint &object)
{
    float fTheta = -body.yaw;
    LocationPoint tempObject = object;
    object.x = cos(fTheta) * (tempObject.x - body.x) - sin(fTheta) * (tempObject.y - body.y);
    object.y = sin(fTheta) * (tempObject.x - body.x) + cos(fTheta) * (tempObject.y - body.y);
}
